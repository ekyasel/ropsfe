...
import { getDoctorYearlySummary, getParameters } from '../../actions/auth';
import { 
  BarChart, 
  Bar, 
  XAxis, 
...
export default function DoctorYearlyReportWidget() {
    ...
    try {
      ...
      if (result.success && Array.isArray(result.data)) {
        const doctors = result.data
          .filter((p: Parameter) => p.param_type === 'DOKTER' && p.is_active)
        ...
        if (doctors.length > 0) {
          ...
          setLoading(false);
          setError("Tidak ada data Dokter yang aktif di Master Data");
        }
      } else {
        setError(result.error || "Gagal memuat daftar dokter");
        setLoading(false);
      }
    } catch (err) {
      console.error("Fetch Doctors Error:", err);
      setError("Gagal menghubungi server untuk data Dokter");
      setLoading(false);
    }
  }, [selectedDoctor]);
  ...
  const loadData = useCallback(async (year: string, doctor: string) => {
    ...
    setError(null);
    try {
      const result = await getDoctorYearlySummary(year, doctor);
      if (result.success) {
        setData(result.data);
      } else {
        setError(result.error || "Gagal memuat data laporan dokter");
      }
    } catch (err) {
  ...
  const handleExportExcel = () => {
    ...
    xlsx.utils.book_append_sheet(wb, ws, "Laporan Dokter Operator");
    xlsx.writeFile(wb, `Laporan_Dokter_Operator_${selectedDoctor}_${selectedYear}.xlsx`);
  };

  if (loading && doctorList.length === 0) {
    return (
      <div className="card" style={{ padding: '4rem', backgroundColor: 'white', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '400px' }}>
        ...
        <p style={{ marginTop: '1rem', color: '#64748b', fontWeight: 500 }}>Memuat konfigurasi dokter...</p>
  ...
  return (
    <div className="card" style={{ backgroundColor: 'white', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '1.5rem', borderBottom: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#fafafa', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h2 style={{ fontSize: '1.25rem', fontWeight: 800, color: '#0f172a' }}>Laporan Per Dokter Operator</h2>
          <p style={{ fontSize: '0.875rem', color: '#64748b' }}>Rincian pasien berdasarkan elektif & cito per dokter operator</p>
        </div>
      ...
      {loading ? (
        <div style={{ padding: '4rem', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '400px' }}>
          ...
          <p style={{ marginTop: '1rem', color: '#64748b', fontSize: '0.9rem' }}>Memuat data {selectedDoctor}...</p>
        </div>
      ) : error ? (
